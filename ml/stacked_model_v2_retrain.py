# ml/stacked_model_v2_retrain.py
import pandas as pd
import numpy as np
...
SCALER_PATH = "./models/scaler_15m.pkl"
META_MODEL_PATH = "./models/btc_long_stacked_v2_model_15m_valid.pkl"
...
# === Параметры (лучший рабочий сетап) ===
SEQUENCE_LENGTH = 90      # ✅ Проверено: работает лучше 120
MAX_HISTORY = 20000       # ✅ Не слишком много, не слишком мало
HOLD_CANDLES = 4          # 60 минут удержания
TEST_SIZE = 72            # 3 дня валидации
TARGET_THRESHOLD = 1.003  # +0.3% — хороший баланс между частотой и силой сигнала

print("🔍 Обучение стекинг-модели: LSTM + XGBoost → LightGBM (meta) на 15m (режим retrain)")
...
# === 12. Финальный прогноз (на валидационной выборке!) ===
...
print(f"\n📊 Результаты: Accuracy={acc:.3f}, Precision={prec:.3f}, AUC={auc:.3f}")
...
log_entry = {
    ...
    "model": "btc_long_stacked_v2_retrain",
    "accuracy": acc,
    "precision": prec,
    "recall": rec,
    "auc": auc,
...
print("✅ Все компоненты модели сохранены. Готово к дообучению!")